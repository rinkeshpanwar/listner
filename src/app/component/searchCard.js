...
function SearchCard(props) {
  ...
  return (
    <div className='w-full'>
        ...
        <div className='mt-3 flex items-center' onClick={() => navigate(`/question/${props?.id}`)}>
            <div className='line-clamp-1 space-x-2'>
                {
                    props?.tags?.map((element) => 
                    <div className='font-primary cursor-pointer text-sm max-w-[100px] truncate inline-block px-2 py-1 bg-slate-200 rounded-sm text-indigo-600 '>{element?.tagName}
                    </div>)
                }
            </div>
